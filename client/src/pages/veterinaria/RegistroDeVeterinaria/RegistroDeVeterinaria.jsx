...
export default function RegistroDeVeterinaria() {
  ...
  if (typeof form.lat !== "number" || isNaN(form.lat) ||
      typeof form.lng !== "number" || isNaN(form.lng)) {
    setErrorStep4("La dirección seleccionada no es válida. Volvé al paso 1 y seleccioná una dirección de la lista.");
    return;
  }
               ...
               <div className={styles.listaItems}>
                  ...
                  <div key={prof.id} className={styles.subCard}>
                    {profesionales.length > 1 && (
                      <button onClick={() => eliminarProfesional(index)} className={styles.btnEliminar} title="Eliminar"></button>
                    )}
                    <Input
