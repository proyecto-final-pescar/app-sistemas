...
export const recibirWebhook = async (req, res) => {
  try {
    ...
    let turnoConfirmadoAhora = false;
    ...
    const pagoGuardado = await prisma.$transaction(async (tx) => {
      ...
      if (turno.estado_turno.nombre !== 'confirmado') {
        ...
        turnoConfirmadoAhora = true;
      }

      return pagoFinal;
    });

    // 8. Aviso al tutor (despues de la transaccion, para no notificar algo que fallo)
    if (turnoConfirmadoAhora) {
